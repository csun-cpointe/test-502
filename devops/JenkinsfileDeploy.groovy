            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Test IAM Role Service Account authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Test IAM Role Service Account failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test IAM Role Service Account deploying"
                ...
                slackSend color: "good",
                        message: "Test IAM Role Service Account deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test IAM Role Service Account failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test IAM Role Service Account shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test IAM Role Service Account failed to shut down properly"
            throw err
        }
    }
}
